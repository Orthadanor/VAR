...
class VQVAE(nn.Module):
    ...
    def forward(self, inp, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        f_hat, usages, vq_loss, commitment_loss, codebook_loss = self.quantize(self.quant_conv(self.encoder(inp)), ret_usages=ret_usages)
        return self.decoder(self.post_quant_conv(f_hat)), usages, vq_loss, commitment_loss, codebook_loss
    # ===================== `forward` is only used in VAE training =====================
    ...
    def idxBl_to_img(self, ms_idx_Bl: List[torch.Tensor], same_shape: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        ...
        for idx_Bl in ms_idx_Bl:
            l = idx_Bl.shape[1]
            pn = round(l ** 0.5)
            ms_h_BChw.append(self.quantize.embedding(idx_Bl).transpose(1, 2).view(B, self.Cvae, pn, pn)) # (B, l, Cvae) -> (B, Cvae, l) -> (B, Cvae, pn, pn)
